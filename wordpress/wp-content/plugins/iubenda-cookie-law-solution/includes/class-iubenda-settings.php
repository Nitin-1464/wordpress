...
class Iubenda_Settings {
	...
	/**
	 * Check site is already made setup?
	 *
	 * @return bool
	 */
	private function check_site_is_already_made_setup() {
		...
		$result = array_filter(
			array_column( iubenda()->options, 'configured' ),
			function ( $service ) {
				return ( stripos( $service, 'true' ) === true );
			}
		);
	...
	public function process_actions() {
		...
		switch ( $this->action ) {
			case 'save':
				...
				$status        = isset( $post_data['status'] ) && in_array( (string) $post_data['status'], array_keys( iubenda()->forms->statuses ), true ) ? sanitize_key( $post_data['status'] ) : 'publish';
				$subject       = isset( $post_data['subject'] ) && is_array( $post_data['subject'] ) ? array_map( 'sanitize_key', $post_data['subject'] ) : array();
				$preferences   = array();
				$exclude       = array();
				$legal_notices = array();
	...
	private function services_option() {
		...
		if ( (string) iub_array_get( iubenda()->options['cs'], 'configuration_type' ) === 'simplified' ) {
			$legislation = (string) iub_array_get( iubenda()->options['cs'], 'simplified.legislation' ) === 'all' ? 'GDPR/USPR/LGPD' : strtoupper( iub_array_get( iubenda()->options['cs'], 'simplified.legislation' ) );
			$cs_settings = array(
				array(
			...
			foreach ( $languages as $k => $v ) {
				...
				if ( $code ) {
					...
					if ( null === iub_array_get( $options, 'enableGdpr' ) && '1' === (string) iub_array_get( $options, 'usprApplies' ) && '1' === iub_array_get( $options, 'enableLgpd' ) ) {
						$legislation = 'GDPR/USPR/LGPD';
					} elseif ( null === iub_array_get( $options, 'enableGdpr' ) ) {
						$legislation = 'GDPR';
					} elseif ( '1' === (string) iub_array_get( $options, 'enableLgpd' ) ) {
						$legislation = 'LGPD';
					} else {
						$legislation = 'USPR';
					}
			...
			if ( iub_array_get( $data, "{$product_name}_status" ) && 'true' === (string) iub_array_get( $data, "{$product_name}_status" ) ) {
				...
				if ( in_array( $product_key, array( 'pp', 'cs', 'tc' ), true ) ) {
					...
					foreach ( $languages as $lang_id => $lang_name ) {
						...
						$parsed_code = array_filter( iubenda()->parse_configuration( stripslashes_deep( $code ) ) );
